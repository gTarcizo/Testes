...
for (let i = 0; i < faturamentos.length; i++) {
  ...
  for (let j = 0; j < col.length; j++) {
    ...
    tabCell.innerHTML = faturamentos[i][col[j]];
  }
}
...
const maiorFaturamento = () => {
	jsonSemZero.map(Number).reduce(function (a, b) {
	  maior = Math.max(a, b);
	  res.innerHTML = `O maior faturamento foi de: ${Math.max(a, b)}`
	  return Math.max(a, b)
	});
}
...
const menorFaturamento = () => {
	jsonSemZero.map(Number).reduce(function (a, b) {
	  res.innerHTML = `O menor faturamento foi de: ${Math.min(a, b)}`
	  return Math.min(a, b);
	});
}

const acimaMedia = () => {
	res2.innerHTML = `O total de ${acima.length} dias faturaram valores acima da média mensal <br/> e <br/> Os dias: ${acima} faturaram valores acima da média mensal`
}
